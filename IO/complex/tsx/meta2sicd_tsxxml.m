...
for pol_i=1:pol_bands
    ...
    % Noise
    % As with DRateSFPoly, for noise computations, we ignore any
    % changes over the azimuth dimension, since its reasonably stable.
    num_noise_est=str2double(xp.evaluate(...
        'count(level1Product/noise/imageNoise)',...
        xml_domnode));
    center_noise_est=num2str(ceil(num_noise_est/2)); % Pick the center
    num_noise_est_coefs=str2double(xp.evaluate(...
        ['count(level1Product/noise/imageNoise[' ...
        center_noise_est ']/noiseEstimate/coefficient)'],...
        xml_domnode));
    noise_est_poly = zeros(num_dop_rate_coefs,1);
    for i=1:num_noise_est_coefs
        noise_est_poly(i)=str2double(xp.evaluate(... % Noise with regard to raw time
            ['level1Product/noise/imageNoise[' ...
            center_noise_est ']/noiseEstimate/coefficient[@exponent="' num2str(i-1) '"]'],...
            xml_domnode));
    end
    noise_est_min = str2double(xp.evaluate(... % Noise reference time
        ['level1Product/noise/imageNoise[' ...
        center_noise_est ']/noiseEstimate/validityRangeMin'],...
        xml_domnode));
    noise_est_max = str2double(xp.evaluate(... % Noise reference time
        ['level1Product/noise/imageNoise[' ...
        center_noise_est ']/noiseEstimate/validityRangeMax'],...
        xml_domnode));
    noise_est_ref = str2double(xp.evaluate(... % Noise reference time
        ['level1Product/noise/imageNoise[' ...
        center_noise_est ']/noiseEstimate/referencePoint'],...
        xml_domnode));
    range_time = linspace(noise_est_min, noise_est_max, 100) - noise_est_ref;
    noise_db = 10*log10(polyval(noise_est_poly(end:-1:1), range_time));
    coords_range_m = 0.5*(range_time + noise_est_ref - range_time_scp)*SPEED_OF_LIGHT;
    noisepoly = polyfit(coords_range_m, noise_db, num_noise_est_coefs-1);
    % NoisePoly doesn't appear to be right.  Perhaps a misinterpretation of
    % the metadata.
